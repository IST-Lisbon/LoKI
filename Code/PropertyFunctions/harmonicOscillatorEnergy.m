...
function stateArray = harmonicOscillatorEnergy(stateArray, property, ...
  argumentArray)
  ...
  for state = stateArray
    if ~strcmp(state.type, 'vib')
      error(['Trying to asign harmonic oscillator energy to non vibrational '...
    ...
    state.energy = Constant.plankReducedInEV*state.gas.harmonicFrequency*...
      (vibLevel+0.5);
  end
